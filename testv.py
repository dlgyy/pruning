...
parser.add_argument('--path', default='./vc16', metavar='DIR',
                    help='path to store')
...
def main():
    ...
    ## read breakpoint
    #model = csresnet50().to(device)
    model = vggnet16_bn().to(device)
   ...
   #submodel.whole_layer_fastprune()
    ...
    #process
    if args.train:
        train_epochs(args, dataloaders, model, criterion, optimizer,args.epochs, pitch=150, store=True)
    if args.valid:
